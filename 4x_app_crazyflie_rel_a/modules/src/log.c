...
void logInit(void)
{
  ...
  logs = (struct log_s *)&logsint;
  logsLen = (int)&logsint.logs_acc[4].address - (int)&logsint.logs_pm[0].type;
  ...
  //Start the log task
  xTaskCreate(logTask, (const signed char *)"log",
...
static struct log_ops * opsMalloc(void);
