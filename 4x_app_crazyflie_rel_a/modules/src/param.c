...
//These are set by the Linker  原代码定义，在链接文件中定义
...
void paramInit(void)
{
  ...
  if(isInit)
    return;
	paramsenserinit(&paramsint);
	paramsimuinit(&paramsint);
	paramscontrinit(&paramsint);
	paramscpuinit(&paramsint);

  params = (struct param_s *)&paramsint;
  paramsLen = (int)&paramsint.params_imu_acc_lpf[2].address - (int)&paramsint.params_sensorfusion6[0].type;
...
static void paramReadProcess(int ident)
{
  ////////////////////////////////////////////////////////////////////////////
	p.header = 0x14;
	memcpy(&p.data[0], params[1].address, sizeof(uint32_t));
	memcpy(&p.data[4], params[2].address, sizeof(uint32_t));
	memcpy(&p.data[8], params[3].address, sizeof(uint32_t));
	memcpy(&p.data[12], params[4].address, sizeof(uint32_t));
	p.size = 17;
	////////////////////////////////////////////////////////////////////////////
	/**************************************************************************
	int id;
