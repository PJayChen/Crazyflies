...
void logpminit(logsvar *plogsint)
{
	plogsint->logs_pm[0].type=LOG_GROUP | LOG_START;
	plogsint->logs_pm[0].name="pm";
	plogsint->logs_pm[0].address=0x0;
	plogsint->logs_pm[1].type=LOG_FLOAT;
	plogsint->logs_pm[1].name="vbat";
	plogsint->logs_pm[1].address=&batteryVoltage;
	plogsint->logs_pm[2].type=LOG_GROUP | LOG_STOP;
	plogsint->logs_pm[2].name="stop_pm";
	plogsint->logs_pm[2].address=0x0;
}
...
void pmInit(void)
{
  ...
  xTaskCreate(pmTask, (const signed char *)"PWRMGNT",
